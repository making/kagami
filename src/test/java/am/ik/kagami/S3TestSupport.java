 ...
 * once per JVM; every test class isolates its objects with its own
...
public final class S3TestSupport {
	...
	static {
		Runtime.getRuntime().addShutdownHook(new Thread(RUSTFS::stop));
	}
	...
	/**
	 * Starts the container if needed and returns its endpoint URL.
	 * @return the http endpoint of the S3-compatible server
	 */
	...
	/**
	 * Registers the {@code kagami.storage.*} and {@code spring.cloud.aws.*} properties
	 * that point the application at the shared rustfs container.
	 * @param registry the registry to add the properties to
	 * @param keyPrefix the key prefix isolating this test class
	 */
