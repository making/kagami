...
Artifacts are stored on the local filesystem by default. To mirror them into Amazon S3 or
any S3-compatible object storage, set the type and bucket:

```properties
kagami.storage.type=s3
kagami.storage.s3.bucket=my-mirror-bucket
# Optional prefix prepended to every key: <prefix>/<repository-id>/<artifact-path>
kagami.storage.s3.key-prefix=kagami
```

Endpoint, region and credentials are configured through Spring Cloud AWS, not `kagami.*`:

```properties
# Any S3-compatible endpoint (leave unset for AWS S3)
...
Credentials, region and endpoint also work through the standard AWS mechanisms (environment
variables, `~/.aws/*`, instance profiles, etc.). With `kagami.storage.type=s3` the diskspace
health indicator and metric are disabled, since they are meaningless for object storage.
